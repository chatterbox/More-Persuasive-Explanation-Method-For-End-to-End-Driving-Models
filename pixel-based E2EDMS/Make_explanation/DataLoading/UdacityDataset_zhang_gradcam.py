...
class UdacityDataset(Dataset):
    ...
    def read_data_single(self, idx):
        ...
        original_img_transform_method = transforms.Compose([transforms.ToPILImage(), transforms.ToTensor()])

        image_transformed = transform_method(image)
        original_image_transformed = original_img_transform_method(original_img)
        del image
        del original_img
        image = image_transformed
        original_img = original_image_transformed
        ...
        return image, All_action, img_name, original_img
    ...
    def read_data(self, idx):
        if isinstance(idx, list):
            ...
            for stack_idx in [0, 1, 3]: # we don't stack timestamp and frame_id since those are string data
