...
def tune_kmeans(count_flag, all_descriptors):
    """
    ...
    plt.ylabel("SSE")
    plt.savefig('./output/tune_kmeans_' + str(count_flag) + ".png")
...
def show_a_pic(y_axis):
    x_axis = []
    for i in range(len(y_axis)):
        x_axis.append(i)
    plt.plot(x_axis, y_axis, color='blue',
             label='Less-FC ', linestyle='solid', linewidth=4)
...
def K_means_zhang_train_iter():
    ...
    dataset_loader_cross_validation_list, test_data_list, max_category_num = build_dataset(config.cross_validation_folder_path_pre, config.train_folder_path,
    ...
    for each_kind_object_numpy in kmeans_group:
        ...
        if count_flag <= 4:
            # print(each_kind_object_numpy, each_kind_object_numpy.shape)
            # exit()
    ...
    for each_test_data in test_data_list:
        processed_test_data = get_processed_df(each_test_data, all_object_codebook)
        test_data_iter = build_iterator(processed_test_data, batch_size, config.device)
        test_iter_list.append(test_data_iter)

    return cross_validation_iter_list, test_iter_list, all_object_codebook
if __name__ == '__main__':
    print(K_means_zhang_train_iter())
